...
%  figure;
hold on
plot(X(:,1),X(:,2), 'k^', 'LineWidth', 5, 'MarkerFaceColor', 'black', 'MarkerSize', 2)%X(:,1)，X(:,2)分别代表的X轴坐标和Y轴坐标
...
for i = 2:row
    ...
    annotation('textarrow',arrowx,arrowy,'HeadWidth',8,'LineWidth', 2,'color',[0,0,1]);%将这两个点连接起来
end
hold off
xlabel('横坐标x')
ylabel('纵坐标y')
